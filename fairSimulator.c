...
int main() {
  ...
  for(int i = 0; i < MAX_GUEST; i++){
    fillGuest(&guest[i], i, half, quarter);
  }
...
/* cleans up the heap; the ticket booth and the rides
 * input:
 * *fair  - pointer to FairType that stores the rides and ticket booth
 *
 * output:
 ...
 *  
 */
void cleanProgram(FairType *fair){
...
void randomizeHeight(GuestType *guest){
  if(guest->age > 12){
    guest->height = rand()%(280-149) + 149;
  }else{
    guest->height = (guest->age*6) + (rand()%(81-72) + 81);
  }
}
...
void randomizeMoney(GuestType *guest){
  ...
  if(guest->age > 10){
    guest->money = moneyAmount[rand()%5];
    guest->money += (moneyAmountOver10[rand()%5]*(guest->age/10));
  }else{
    guest->money = moneyAmount[rand()%5];
  }
 ...
 * *guest - pointer to GuestType that stores the visiting guests
 * i - vistor number
 * half - roughly a half of the MAX_GUEST to assign ages
 * quarter - roughly a quarter of the MAX_GUEST to assign ages
 * 
 * output:
 * none;
 *
 ...
 *  
 */
void fillGuest(GuestType *guest, int i, int half, int quarter){
  guest->numberId = i;

  //Randomized age
  if(i < half){
    guest->age = rand()%(19-13) + 13; 
  }
  if(i >= half && i < quarter + half){
    guest->age = rand()%(12-5) + 5;
  }
  if(i >= quarter + half && i < MAX_GUEST){
    guest->age = rand()%(65-20) + 20;
  }

  randomizeMoney(guest);
  randomizeHeight(guest);

  guest->ticketAmount = 0;
  guest->ridesRidden = 0;
  guest->passCheck = 0;
  guest->admissionCheck = 0;
}
